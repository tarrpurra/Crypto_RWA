# AI + Data Analytics Service

## Purpose

This folder contains the working documentation for the AI + Data Analytics service in AIxRWA.

## Ownership Scope

- allocation engine
- AI reasoning and prompt logic
- Pyth / Hermes ingestion
- quote sampling and market normalization
- risk scoring
- simulations and backtests
- analytics APIs consumed by the website

## Documents

- [setup.md](./setup.md)
- [ImplementationPlan.md](./ImplementationPlan.md)
- [Phase0.md](./Phase0.md)
- [Phase1.md](./Phase1.md)
- [Phase4.md](./Phase4.md)
- [Phase5.md](./Phase5.md)
- [Phase2.md](./Phase2.md)
- [Phase3.md](./Phase3.md)
- [recommendation_model.md](./recommendation_model.md)
- [ai_plan.md](./ai_plan.md)
- [Changes.md](./Changes.md)

## Usage

- start with `setup.md` to prepare the service environment
- use `ImplementationPlan.md` as the build and delivery guide
- use `Phase0.md` as the bootstrap execution guide
- use `Phase1.md` as the market and oracle ingestion execution guide
- use `Phase4.md` as the allocation execution guide
- use `Phase5.md` as the AI reasoning execution guide
- use `Phase2.md` as the portfolio analytics execution guide
- use `Phase3.md` as the deterministic risk-engine execution guide
- use `recommendation_model.md` as the canonical runtime, status, error, and recommendation-output decision contract
- use `ai_plan.md` as the active repair plan for the current risk, allocation, AI, proposal, and endpoint flow
- update `Changes.md` whenever scope, APIs, risk logic, or analytics behavior changes
